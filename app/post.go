...
func fetchPosts(ctx *Ctx, forUser *User, dtFrom time.Time, dtUntil *time.Time) (ret []*Post) {
	query := dbQueryPostsForUser(forUser).And(PostDtMade.GreaterOrEqual(dtFrom))
	if dtUntil != nil {
		query = query.And(PostDtMade.LessOrEqual(dtUntil))
	}
